...
If you want to call the MCP resource route from external tools (like MCP
Inspector or MCP Jam) without a browser session, set `MCP_DEV_TOKEN` and pass
`Authorization: Bearer <token>` plus an `X-MCP-User-Id` header containing the
user id to impersonate. This is intended for development/testing only.
