	...
	return (
		<div className="grid gap-8">
			...
			<section className="border-y border-border/40 py-4">
				...
				<p className="mt-2 text-body-2xs text-muted-foreground">
					Link a new provider to sign in or sync data.
				</p>
		...
		<div className="flex flex-wrap items-center justify-between gap-3">
			<span className="inline-flex items-center gap-2 text-body-sm">
				...
				<span className="text-muted-foreground">
					{connection.link ? (
						<a href={connection.link} className="hover:text-foreground">
							{connection.displayName}
						</a>
					) : (
						<span className="text-foreground">{connection.displayName}</span>
					)}{' '}
